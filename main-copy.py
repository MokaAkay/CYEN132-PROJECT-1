...
class Ball(Entity):
    def __init__(self,xPos,yPos,ballImg):
        ...
        self.bounces=0
        self.direction = randint(0,360) #yet to be implemented 
    ...
    def update(self):
        ...
        if self.yPos <= 0:
                self.yspeed *= -1
        elif self.yPos >= display_height-self.size:
                self.yspeed *= -1

        if self.xPos <= 0:
                self.__init__(display_width/2, display_height/2,None)#reset ball
                # score point to be added later #
        elif self.xPos >= display_width-self.size:
                self.__init__(display_width/2, display_height/2,None)
                self.xspeed = 5
                # score point to be added later #
    ...
    def render(self):
        gameDisplay.blit(self.ballImg,(self.xPos,self.yPos))

######################################
# MAIN PART OF THE PROGRAM
######################################
#adds all of the required entities to the entities list
entities_list = [Paddle(20, 10, 1, True), Paddle(display_width-20, 20, 2, True), Ball(display_width/2, display_height/2,None)]
